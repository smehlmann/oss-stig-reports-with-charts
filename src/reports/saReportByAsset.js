...
async function runSAReportByAsset(auth, inEmassNums, emassMap) {
  try {
    ...
    const headers = [
      ...
      { label: "CAT2", key: "cat2" },
      { label: "CAT1", key: "cat1" },
      { label: "Web Or Database", key: "cklWebOrDatabase" },
    ];
...
function getRow(
  ...
  var dbOrWeb = false;
  if (asset.metadata && asset.metadata.cklWebOrDatabase) {
    dbOrWeb = asset.metadata.cklWebOrDatabase;
  }
